        ...
        TimerTask firstRemainder = new TimerTask() {
            public void run() {
                if (bot.getGame().isActivePhase() && userName.equals(bot.getGame().getCurrentUser()))
                    bot.sendSimpleMessage("Осталось 15 секунд!", bot.getGame().getChatId());
                    bot.sendSimpleMessage("Осталось 15 секунд!", bot.getGame().getUsers().get(bot.getGame().getCurrentUser()));
            }
        };
        TimerTask lastRemainder = new TimerTask() {
            public void run() {
                if (bot.getGame().isActivePhase() && userName.equals(bot.getGame().getCurrentUser()))
                    bot.sendSimpleMessage("Осталось 5 секунд!", bot.getGame().getChatId());
                    bot.sendSimpleMessage("Осталось 5 секунд!", bot.getGame().getUsers().get(bot.getGame().getCurrentUser()));
            }
        };
